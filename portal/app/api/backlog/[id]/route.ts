...
export async function PUT(request: NextRequest, { params }: Params) {
  ...
  if (body.prd_section !== undefined)
    allowed.prd_section = body.prd_section || null;
  ...
  if (body.reviewer_id !== undefined)
    allowed.reviewer_id = body.reviewer_id || null;
  if (body.deliverable_id !== undefined)
    allowed.deliverable_id = body.deliverable_id || null;
    ...
    if (advancingStatuses.includes(body.status)) {
      ...
      if (currentItem?.blocked_by) {
        ...
        if (blockerItem && blockerItem.status !== "done") {
          ...
          return NextResponse.json(
            {
              error: `Blocked by BL-${blockerItem.seq_id}: "${
                blockerItem.title
              }" (${blockerItem.status.replace("_", " ")}). Complete it first.`,
              blocker: {
  ...
  const { data, error } = await supabase
    ...
    .eq("id", id)
    .select(
      "*, assignee:user_profiles!assigned_to(full_name, avatar_url, dev_role), blocker:backlog_items!blocked_by(id, title, status, seq_id)"
    )
    .single();
  ...
  if (body.status === "review" && prev?.status !== "review") {
    ...
    if (fullItem?.reviewer_id) {
      ...
      if (reviewer) {
        ...
        if (reviewer.email) {
          const prefs = reviewer.notification_preferences as {
            email_on_feedback?: boolean;
          } | null;
          if (prefs?.email_on_feedback !== false) {
            const APP_URL =
              process.env.NEXT_PUBLIC_APP_URL ?? "https://horuseye.app";
        ...
        log({
          ...
          resource_id: id,
          action: `Review requested: BL-${fullItem.seq_id} → ${
  ...
  if (
    ...
    body.assigned_to !== prev?.assigned_to
  ) {
    await supabase.from("backlog_activity").insert({
      backlog_item_id: id,
      user_id: user.id,
  ...
  if (body.status === "done" && data.deliverable_id) {
    ...
    const allDone = siblingItems?.every(
  ...
  if (changedFields.length > 0) {
    ...
    if (body.assigned_to !== undefined)
      actionParts.push(
        `assigned → ${data.assignee?.full_name ?? "unassigned"}`
      );
    if (body.sprint_id !== undefined)
      actionParts.push(body.sprint_id ? "moved to sprint" : "moved to backlog");
    if (body.priority !== undefined)
      actionParts.push(`priority → ${body.priority}`);
    if (body.actual_hours !== undefined)
      actionParts.push(`logged ${body.actual_hours}h`);
    if (body.epic !== undefined)
      actionParts.push(`epic → ${body.epic ?? "none"}`);

    log({
      event_type:
        body.status === "done" ? "checklist.check" : "checklist.update",
      severity: "info",
      user_id: user.id,
      resource_type: "backlog_item",
      resource_id: id,
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  if (fileDisplayName) {
    notifyAdmins(
      ...
      `The backlog item "${
        item?.title ?? id
      }" was deleted, which was associated with the file "${fileDisplayName}".`,
      undefined
    );
  }
